...
class PersistBlock
  ...
  def create_first_block
    return unless File.read(LEDGER_LOACTION).empty?
  ...
  def last_persisted_block
    return if File.read(LEDGER_LOACTION).empty?
    last_line = File.readlines(LEDGER_LOACTION).last
