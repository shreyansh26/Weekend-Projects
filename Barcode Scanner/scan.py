...
# On hor_data, 1 is black but in RGB, black is 0
...
print("Bit Width: {}".format(bit_width))
...
VALUES = [value for value in CODE128_CHART[2::8]]
CODE128 = dict(zip(SYMBOLS, VALUES))
...
for sym in symbols:
    try:
        if CODE128[sym] == "Start":
            continue
        if CODE128[sym] == "Stop":
            break

        str_out += CODE128[sym]
        # print(" ", sym, CODE128[sym])
    except Exception as e:
        continue

print("Decoded Value: {}".format(str_out[:-1]))
